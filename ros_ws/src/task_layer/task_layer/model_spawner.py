...
def make_spawn_command(params: dict) -> list[str]:
    # Gazebo Classic (11) uses gazebo_ros spawn_entity.py with positional CLI flags,
    # not the gz-sim style '--ros-args -p key:=val'.
    command = [
        'ros2', 'run', 'gazebo_ros', 'spawn_entity.py',
        '-file', str(params['file']),
        '-entity', str(params['name']),
...
class SpawnModel(Node):
    ...
    def run(self):
        ...
        if return_code != 0:
            self.get_logger().error('gazebo_ros spawn_entity.py failed with code %d' % return_code)
        return return_code
